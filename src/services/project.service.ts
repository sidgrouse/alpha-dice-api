...
export class ProjectService {
    ...
    async tryAddProject(name: string, url: string, details: string, itemsWithK1: {name: string, invoiceK1Amount: number}[]): Promise<boolean> {
    ...
    async getAllAvailableItems(): Promise<ItemDto[]> {
        ...
        return items
            ...
            .map(itm => new ItemDto(itm.id, itm.name, itm.project.name, itm.project.status)); //TODO: add details to order
    } 
}