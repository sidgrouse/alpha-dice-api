...
export class AddInvoiceTgSceneController {
  ...
  @Command('save')
  async onSave() {
    await this._invoiceBuffer.forEach(
      async (itm) => await this._invoiceService.addInvoice(itm),
    );
    //console.log(ret);
    return 'saved';
  }
  ...
  async onMessage(@Message() messageObject: any) {
    ...
    invoiceStrs.forEach((str) => {
      ...
      const dto = new CreateInvoiceDto(
        invoiceElements[0],
        Number.parseInt(invoiceElements[1]),
      );
      this._invoiceBuffer.push(dto);
    });
    return this._invoiceBuffer
      .map((itm) => `${itm.userId} (${itm.pledjeId})`)
      .join('\n');
  }
}
